...
echo "[2/6] Firebase rules JSON check"
python -m json.tool firebase_rules.json >/dev/null

echo "[3/6] Index module include check"
...
echo "[4/6] Index module marker check"
...
echo "[5/6] Security guardrail check"
...
grep -q "onclick=\"triggerOptIngCheck()\"" index.html
