...
// 아이콘 매칭이 확실하지 않은 재료는 기존 방식(이모지)으로 노출
// 필요시 이름만 추가하면 해당 재료는 이미지 대신 이모지를 사용합니다.
var ICON_EXCLUDE_INGS=new Set(['참치캔','참기름']);
INGS=INGS.map(function(i){
  if(!ICON_EXCLUDE_INGS.has(i.n))return i;
  return {n:i.n,e:i.e,c:i.c,ic:''};
});
